# Shaylea Smith
<h2> Welcome to My Profile

<h2> Personal Information </h2>

<p> Shaylea Smith </p>
<p> Student #: s3795647 </p>
<p> Email: s3795647@student.rmit.edu.au </p>

<p> I am a 30 year old Australian born and breed have lived in a few different areas within Queensland but based in Ipswich with my husband and 2 children since 2009. I am a very busy person I work four days a week as a Principal Customer Service Officer at Department of Transport and Main Roads have been there since 2007 where I completed my Cert III in Business Administration. I take on many roles within the team which keeps me vey busy. </p>

<p> I also have my own hobby business which I bake and decorate custom cakes for clients. I help my dad and sister run their own businesses as well, I usually help with the IT stuff and anything else they need. I also manage my sons football team "Go Eagles" which consists of twelve 9 year old boys I love watchin them grow and learn so many skills. I try to attend the gym 6 nights a week to keep fit and healthy. My work colleges called me the modern day super Woman when they heard I was also adding a University degree to my already hectic life, but I love a challenge. </p>

<p> I enjoy spendin my free time with my family and visiting my husbands family in South Australia, which we have driven 3 times now and back but can not wait for the kids to be older to be able to enjoy it more and see the marvelous sites the trip has to offer and remember them. </P>


<h2>  Interest in IT </h2>

<h3> What is your interest in IT? When did your interest start? Was there a particular event or person that sparked your interest? Outline your IT experience (if any) </h3>
  <p>  My intereest a\has always been there for everything IT and how things need IT for the world to progress and move forward. I guess my dad was the person that sparked my interest in IT when he showed me how Goole Ads work for his business and it just grew from there with wantin to improve his business in the IT world and makin it all that it could be. </p>
  
<h3> Why did you choose to come to RMIT? </h3>
  <p> I was recommended RMIT by my Student Advisor through Open University, so far I am happy with RMIT I have heard many good things and reat reviews about RMIT.  </p>

<h3> What do you expect to learn during your studies? </h3>
  <p> I expect to learn many thins and many skills to be able to proress my career to find my ideal job. I hope to learn more skills and knowledge about IT and how things work.  </p>
  
  
  
<h2>  Ideal Job </h2>
<h3> <p> Advertisment </p> </h3>
<p>Senior Business Analyst - MS Dynamics SCV</p>
<p> <a> https://www.robertwalters.com.au/information-technology/jobs/businessanalyst/1261106-seniorbusinessanalyst-msdynamicsscv.html </a> </p>
  <p>An excited long-term opportunity for a Senior Business Analyst – MS Dynamics SCV to join a leading Government organization on an enterprise wide integration. The successful candidate must have extensive experience working with Single Customer View, MS Dynamics, it’s configuration options, vendors and a strong stakeholder management. The project will focus on data migration, security and systems integration spanning over the next 2 years.</p>

<p>Key Responsibilities. </p>
<p>The IT Business Analyst will:</p>
<p>- Build business cases for the inception of the project and help steer the Project Manager in delivery</p>
<p>- Conduct and facilitate requirements gathering with both business and technical stakeholders</p>
<p>- Developing business requirements documents and other project artefacts</p>
<p>- Creating process maps and designing the as/is to/be processes</p>
<p>- Co-ordinate and manage the UAT and test plans</p>
<p>- Support in vendor decision making</p>


<p>Key Requirements</p>
<p>The IT Business Analyst will have:</p>
<p>- 10+ years formal Business Analyst experience</p>
<p>- 6+ years’ experience with Microsoft Dynamics and it’s configuration options</p>
<p>- Single Customer View experience is mandatory</p>
<p>- Outstanding verbal, written and presentation skills</p>
<p>- Prior Government experience is highly desired </p>
<p>- Industry relevant certifications as well as technical qualifications </p>

<h3> <p> Description of Position </p> </h3>
<p> This position is for someone to build the business and cases and its process moving forward including supporting the business with developing processes and requirements with stakeholders. Managing the test plans and supporting the vendor. Liase and assist project manager. Conduct and faciliate technical and business requirements.   </p>

<h3> <p> Description of Skills, Qualifications and Experiences Needed </p> </h3>
<p> 1. 10 + years experience in a BA role,  </p>
<p> 2. Experience with MD and it's configuration options  </p>
<p> 3. Outstanding communication in all aspects  </p>
<p> 4. Government experience  </p>
<p> 5. Industry qualifications  </p>


<h3> <p> Description of Skills, Qualifications and Experiences Currently Have </p> </h3>
<p> I currently only have Government experience  </p>
<p> Great Customer interacton  </p>
<p> Ability to solve problems  </p>
<p> Able to work with stakeholders to produce an outcome.  </p>

<h3> <p> Plan to Obtain Skills, Qualifications and Experince </p> </h3>
<p> My plan moving forward is to complete my degree, obtain some experience in the field as a BA and educate myself on MD and any other useful programs. </p>


<h2>  Personal Profile </h2>

   <h3> <p> Myers-Briggs Test Result: </p> </h3>

   <p> Personality type: “The Consul” (ESFJ-A)
Individual traits: Extraverted – 81%, Observant – 77%, Feeling – 64%, Judging – 97%, Assertive – 56%
Role: Sentinel
     Strategy: People Mastery </p>

  <h3> <p> Learning Style Test Result: </p> </h3>
  <p> Your Scores:</p>
<p> Auditory: 25%   Visual: 50%   Tactile: 25% </p>
<p> You are a Visual learner! Check out the information below, or view all of the learning styles. </p>

<p> Visual </p>
<p> If you are a visual learner, you learn by reading or seeing pictures. You understand and remember things by sight. You can picture what you are learning in your head, and you learn best by using methods that are primarily visual. You like to see what you are learning. </p>

<p> As a visual learner, you are usually neat and clean. You often close your eyes to visualize or remember something, and you will find something to watch if you become bored. You may have difficulty with spoken directions and may be easily distracted by sounds. You are attracted to color and to spoken language (like stories) that is rich in imagery. </p>

   <h3> <p> Personality Test Result: </p> </h3>
  <p> Textual summary of your report </p>
<p> The Big Five Factors	Describing a low range scoring person...	Percentile	Range	Describing a high range scoring person... </p>
<p> Openness to experience - Traditionalist • down-to-earth • practical • conservative	27	Low	Imaginative • open-minded • experimental </p>
<p> Conscientiousness (Work Ethic) -Spontaneous • organised • prefers flexible plans	98	Very high	Conscientious • disciplined • efficient • well organised </p>
<p> Extraversion - Reserved • formal • serious • quiet	88	High	Outgoing • friendly • assertive • likes working with others </p>
<p> Agreeableness - Hard-headed • sceptical • competitive • proud	73	High	Compassionate • eager to please • good natured </p>
<p> Natural reactions - Not easily upset in stressful situations • relaxed	1	Very low	Experiences negative emotional reactions and feelings of anxiety • prone to worry </p>

<h3> What do the results of these test mean for you? </h3>
  <p> I belive these results indicate that I am a visual learner  that is well organised and that has a high work ethic and is disciplined, but hard headed as I like to take the lead. I am very observant extrovert but can get people to work together well.  </p>
  
<h3> How do you think these results may influence your behaviour in a team? </h3>
  <p> I think these results will make me more aware of the type of person I can be with takin over tasks and not letting everyone else have an opinion or voice. I think it will influence my behaviour by allowing people to contribute together instead of taking charge.   </p>
  
<h3> How should you take this into account when forming a team? </h3>
  <p> I should make myself aware of other people and the type of people I form a group with and avoid groups that are disorganised and with all extraverts as we may well just all clash.  </p>

<h2>  Project Idea </h2>

<p> Learning App for Children with Learning Difficulties </p>

<h3> <p> Overview </p> </h3>
<p> I would like to develop an app for children with learning difficulties such as Dyslexia, Irlen, central auditory processing disorder and deafness just to name a few. I want the app to be widely accessible to those that have multiple difficulties with learning. The idea is to be able to have one app that caters to multiple difficulties rather then just 1 app for individual learning disabilities instead of needing to download multiple apps on devices. The app would include the ability to evaluate the child’s needs as they progress through the learning levels.  </p>

<h3> <p> Motivation </p> </h3>
<p> My motivation behind the idea of this app is my daughter and other children like her. She was recently diagnosed with Iren, central auditory processing disorder and partial deafness which all impacted her ability to learn the same way as her peers. She had fallen behind in school and after doing much research and multiple app downloads to find that many where either not the right fit for her or that she has to use multiple apps to help her improve. We also need to have her formally assessed every 12months to see her progress, as this is outside of her comfort zone, she feels anxious and while she may be able to do something at home finds it more difficult under assessment criteria.  </p>

<h3> <p> Description </p> </h3>
<p> After speaking with a few teachers, it was made more evident that there is a higher increase in the number of children that are being diagnosed with multiple learning difficulties and having one app that could help multiple children would be extremely beneficial.</p> 
<p> My app will consist of multiple learning features such as the following: </p>
        <p> -	Colour tint overlay for children suffering from Irlen. </p>
        <p> -	Ability to practice learning techniques and the correct way to form letters and numbers. </p>
        <p> -	Sign language for children with deafness or partial deafness including a section on how to learn sign language. </p>
        <p> -	The option to change the tone, pitch and sex of the voice that is speaking to suit the child’s needs. </p>
        <p> -	Speech development. </p>
        <p> -	Teaching different sounds of letters and the multiple sounds letters can make. </p>
        <p> -	Learning phonics. </p>
        <p> -	Identify upper- and lower-case letters. </p>
<p> While I have listed many of the features above, I will also be including a section for children to learn and practice techniques on calming down in stressful situations that may make them anxious and want to lash out or play up. In this section it will include calming down music, breathing techniques to practice and meditation to reduce their anxiety levels faster to be able to return to their task more quickly. </p>
<p> The app will also include a section for parents and teachers to be able to do assessments on the child in the comfort of the home and then send the results to the specialist in that field for them to access the results and see the development of the child. In the parent and teacher section it will also include information for them and where they can access more support to assist their child and places to find more information to understand the child’s difficulties a little more. </p>
<p> By being able to screen the child more often will mean that if the needs change, they can pick it up sooner rather then later and get on top of it for the benefit of the child. It will also give teachers and parents the information they need on what areas the child may need more attention on. </p>
<p> As the child progresses each level and completes it, they will receive a certificate that can be emailed and printed for them to keep and see the progress they are making. While the app will be designed to help improve the ability of learning for each individual child it will be done in a fun educational way, so the child does not get bored and not want to use the app but instead keep them focused and interested. This will allow the child to think they are playing when in actual fact they are learning at the same time. I believe this will help develop the fundamental learning to help each child to succeed. 
 </p>

<h3> <p> Tools and Technologies </p> </h3>
<p> Tools that would be needed to build and develop such an app would include an app builder such as firebase or appetizer as I am familiar with both of these. Building the app would require specific programming to allow the learning to flow easily. The app would require access control and permissions to use the devices microphone and speaker and other certain functions. The use of mobile development software would be required to carry out the correct coding. For the different graphics it will also need a graphical user interface for each component. </p>

<h3> <p> Skills Required </p> </h3>
<p> Skills required to develop such an app would first include the skills of those that have studied the field of each difficulty to ensure the content that is put into the app will be beneficial to the user and their needs. It will also require the skills of professionals that do the assessments to ensure the right screenings are used to achieve the best outcomes. Other skills would be being able to develop complex coding to ensure each component runs without errors. A specific program would need to be established to make sure the flow of the app was running at the best possible level. The feasibility of these skills would not be hard to find as there are many specialists out there that I am sure would be more than willing to contribute and as there are many individual apps for each learning ability, I am sure that it would not be hard to find the programing and coding skills required.  </p>

<h3> <p> Outcome </p> </h3>
<p> If the app was successful it would make a world of difference to children with difficulties in learning to stay up to date with their peers. I also believe that for parents and teachers it will make monitoring a child’s development more easily, which will allow the child to excel in areas and assist the child in the areas they are struggling. The biggest impact this app will have is helping those that need it the most and impacting their lives so the do not constantly feel like they are being left behind by not understanding the work the same as their peers. This app will also allow the screening to be done more often and give the ability to see when the child’s needs change. This app will replace the need for multiple apps thus reduce the cost of purchasing multiple apps that are not focused on multiple learning difficulties at once. </p>
